...
public class ClubUserDetailsService implements UserDetailsService { //ClubMember 가 ClugAuthMemberDTO 라는 타입으로 처리된 가장 큰 이유는 사용자의 정보를 가져오는 핵심적인 역할을 하는 UserDetailsService 라는 인터페이스 ㄸ문입니다. 스프링 시큐리티의 구조에서 인증을 담당하는 AuthenticationManager 는 내부적으로 UserDetailsService 를 호출해서 사용자의 정보를 가져옵니다. 이 코드에서 주목해서 봐야하는 점은 @Service 어노테이션을 활용하여 자동으로 스프링에서 빈으로 처리될 수 있게 되어 있다는 점과 loadUserByUserName( )에서는 별도의 처리 없이 그저 로그를 기록하고 있다는 점입니다.
