...
public class MemoRepositoryTest {
	...
	@Test
	public void 데이터단건조회() {
		Optional<Memo> result = memoRepository.findById(1); //키값으로 조회. 조회결과를 optional로 반환
	...
	@Test
	public void 데이터전체조회() {
		List<Memo> list = memoRepository.findAll(); //전체목록은 리스트로 반환
		for(Memo memo : list) {
			System.out.println(memo);
		}
	}
